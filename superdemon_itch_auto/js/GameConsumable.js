...
import { UPGRADES_DB, WARDENS_NEEDED_FOR_RITUAL } from './Constants.js';
import { EntityManager } from './EntitiesConsumable.js';
...
export const GameConsumable = {
    ...
    // Upgrades are now Ammo Counts (Integers)
    ...
    start() {
        ...
        this.em.queueSpawn(2); this.updateUI();
    },
    ...
    // REMOVED: triggerLevelUp, selectUpgrade
    ...
    executeDash() {
        ...
        if (this.upgrades.echo > 0) {
            this.upgrades.echo--;
            echoActive = true;
        }

        const lineLife = 30 + (echoActive ? 20 : 0);
        this.em.slashLines.push({ x1: startX, y1: startY, x2: ex, y2: ey, life: lineLife, width: 15, lethal: echoActive });
        ...
        for (let i = this.em.mines.length - 1; i >= 0; i--) {
            ...
            if (this.lineCircleCollide(startX, startY, ex, ey, m)) {
                ...
                if (this.upgrades.phase > 0) {
                    this.upgrades.phase--; // Consume Phase
                    this.em.mines.splice(i, 1);
                    this.em.spawnText(m.x, m.y, "EATEN", `hsl(${this.hue}, 100%, 50%)`);
                    this.em.spawnConfetti(m.x, m.y, `hsl(${this.hue}, 100%, 50%)`, 8);
                    // Reward Score instead of XP
                    this.score += 500;
                }
        ...
        // CONSUMABLE: Explosion
        if (this.upgrades.explosion > 0) {
            this.upgrades.explosion--; // Consume Explosion
            const radius = 150; // Fixed radius or slightly larger
    ...
    killEnemy(index, source) {
        ...
        if (deadEnemy.type === 4) { // Boss
            ...
            // WARDEN CACHE: Spawn 3-5 random pickups
            const count = 3 + Math.floor(Math.random() * 3);
            const types = ['explosion', 'range', 'echo', 'phase', 'nanites', 'voltage'];
            for (let i = 0; i < count; i++) {
                const type = types[Math.floor(Math.random() * types.length)];
                this.em.spawnPickup(dx + (Math.random() - 0.5) * 50, dy + (Math.random() - 0.5) * 50, type);
            }
        ...
        if (deadEnemy.type === 5 && deadEnemy.lives > 0) {
            ...
            AudioSys.deflect();
            deadEnemy.x += (Math.random() - 0.5) * 300;
            deadEnemy.y += (Math.random() - 0.5) * 300;
            deadEnemy.x = Math.max(0, Math.min(this.width, deadEnemy.x));
            deadEnemy.y = Math.max(0, Math.min(this.height, deadEnemy.y));
            return false;
        }
        ...
        // GLITCH CHIP: Every 10 Combo
        if (this.combo % 10 === 0) {
            const types = ['explosion', 'range', 'echo', 'phase', 'nanites', 'voltage'];
            const type = types[Math.floor(Math.random() * types.length)];
            this.em.spawnPickup(dx, dy, type);
            this.em.spawnText(dx, dy - 50, "GLITCH CHIP", "#0ff");
        }

        // CONSUMABLE: Nanites
        if (this.upgrades.nanites > 0) {
            // Chance to spawn nanites? Or always?
            // Let's make it consume 1 charge per kill to spawn a swarm
            this.upgrades.nanites--;
            const spawnCount = 5;
        ...
        // CONSUMABLE: Range (Passive? Or consume?)
        // Let's make Range passive if you have ammo, consume on use?
        // "Stun Radius" uses range.
        // Let's consume 1 range charge per kill to trigger the stun wave?
        // Or maybe Range is just a stat that degrades?
        // I'll make it consume 1 charge to boost the stun radius significantly.
        let rangeMult = 0;
        ...
        const stunRadius = 300 * (1 + rangeMult * 0.5);
        ...
        // No more Data drops for XP
        // Just score particles?
        // spawnData was used for XP. I'll skip it or replace with score text.
    ...
    update(dt) {
        ...
        let spawnRate = 0.01 + (this.score / 100000);
        ...
        if (this.upgrades.voltage > 0 && this.aim.active) {
            this.player.aimTimer += scaledDt;
            const radius = 200; // Fixed radius
            if (Math.floor(this.player.aimTimer) % 5 === 0) {
                // Consume Voltage slowly? Or just require it?
                // Let's consume 1 charge every 30 frames (0.5s)
                if (Math.floor(this.player.aimTimer) % 30 === 0) {
                    this.upgrades.voltage--;
                    this.updateUI();
                }

                if (this.upgrades.voltage > 0) {
                    this.em.list.forEach((e, i) => {
                        if (Math.hypot(e.x - this.player.x, e.y - this.player.y) < radius) {
                            this.em.spawnConfetti(e.x, e.y, '#fa0', 1); e.x += (e.x - this.player.x) * 0.1; e.y += (e.y - this.player.y) * 0.1;
                        }
                    });
                }
            }
        ...
        let mineRate = 0.005;
        if (this.wardensKilled === 0) mineRate = 0;
            ...
            if (l.lethal && l.life > 5) {
                for (let j = this.em.list.length - 1; j >= 0; j--) {
                    ...
                    if (this.lineCircleCollide(l.x1, l.y1, l.x2, l.y2, e)) {
                        // Echo Slash Logic with Blocking
                        let blocked = false;
    ...
    updateUI() {
        ...
        // XP Bar is hidden via CSS, no need to update
        ...
        for (const [key, val] of Object.entries(this.upgrades)) {
            if (val > 0) {
                const u = UPGRADES_DB[key];
                const div = document.createElement('div');
                div.className = 'perk-icon';
                // Show Icon + Ammo Count
                div.innerHTML = `${u.icon}<div class="ammo-count">${val}</div>`;
                div.title = u.name;
