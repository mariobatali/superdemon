...
export class EntityManager {
    ...
    spawnBoss() {
        ...
        // Removed "WARDEN ALERT" popup
        // document.getElementById('boss-warning').style.display = 'block';
        // setTimeout(() => document.getElementById('boss-warning').style.display = 'none', 3000);

        // Add massive grid distortion instead
        this.game.grid.applyForce(this.game.width / 2, this.game.height / 2, 1500, 300);
        // AudioSys.warn();
    ...
    update(dt) {
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            if (p.type === 'data') {
                ...
                if (dist < 300) { const force = (400 - dist) / 20; p.vx += (dx / dist) * force * timeScale; p.vy += (dy / dist) * force * timeScale; }
                if (dist < this.game.player.radius + p.size + 10) {
                    AudioSys.collect();
                    this.game.player.xp += 10;
                    if (this.game.player.xp >= this.game.player.nextLevelXp) this.game.triggerLevelUp();
                    p.life = 0;
                    this.game.updateUI();
                }
                p.vx *= 0.92; p.vy *= 0.92;
        ...
        for (let i = this.list.length - 1; i >= 0; i--) {
            ...
            // WARDEN SHIELD ROTATION
            if (e.type === 4) {
                e.shields = e.shields.map((angle, i) => angle + (0.02 * (i % 2 === 0 ? 1 : -1)) * timeScale);
            }

            // Singularity Pull (Moved from Game.js)
            if (e.type === 3) {
                if (e.warmupTimer > 0) {
                    e.warmupTimer -= timeScale;
                } else {
                    const dist = d; // d is calculated above
                    const pullRange = 450;
