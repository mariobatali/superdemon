...
export const Game = {
    ...
    update(dt) {
        ...
        const mobCap = (12 + (this.score / 300)) * (this.combo > 50 ? 2 : 1); // Double cap at 50+ Combo
        ...
        if (!this.bossActive && !this.finalBossActive) {
            ...
            const shooterCap = (2 + Math.floor(this.score / 5000)) * (this.combo > 50 ? 2 : 1);
            ...
            // Hard Cap of 3 (6 at 50+), Slow Rate
            const gravityCount = this.em.list.filter(e => e.type === 3).length;
            const gravityCap = this.combo > 50 ? 6 : 3;
            if (gravityCount < gravityCap && this.score > 250) {
            ...
            const glitchCap = this.combo > 50 ? 4 : 2;
            if (glitchCount < glitchCap && this.score > 500) {
            ...
            const shieldedCap = this.combo > 50 ? 6 : 3;
    ...
    triggerPrismNova(x, y) {
        ...
        this.em.list.forEach(e => {
            const ex = e.visualX || e.x; const ey = e.visualY || e.y;
            const dx = ex - x; const dy = ey - y; const d = Math.sqrt(dx * dx + dy * dy);
            if (d < 550) {
                if (e.type !== 3) { // Singularity Immunity to Nova Push
                    e.vx += (dx / d) * 15; e.vy += (dy / d) * 15; e.stunned = 120;
                }
            } // Reduced range (was 800)
        });
    },
    ...
    updateUI() {
        ...
        // COMBO COLOR SCHEME
        let cColor = '#fff'; // Tier 0 (0-9)
        if (this.combo >= 50) cColor = `hsl(${this.hue}, 100%, 50%)`; // Tier 3+ (Rainbow)
        else if (this.combo >= 25) cColor = '#f0f'; // Tier 2 (25-49)
        else if (this.combo >= 10) cColor = '#0ff'; // Tier 1 (10-24)
    ...
    draw() {
        ...
        try {
            ...
            // Grid
            this.grid.draw(this.ctx, this.hue, this.width / 2, this.height / 2, this.combo, this.victoryMode, this.distortionLevel);
            ...
            this.em.list.forEach(e => {
                ...
                if (e.type === 4) {
                    ...
                    if (e.warmupTimer <= 0) {
                        this.ctx.beginPath(); this.ctx.arc(dPos.x, dPos.y, 70, 0, Math.PI * 2); // Reduced to 70 (was 100)
                    ...
                    if (e.hasShield) {
                        ...
                        // 50% Coverage = PI (180 degrees)
                        this.ctx.arc(e.x, e.y, e.radius + 8, e.shieldAngle - (Math.PI / 2), e.shieldAngle + (Math.PI / 2));
                        this.ctx.stroke();
                        this.ctx.restore();
                    }
                }
            ...
            for (let i = 0; i < this.player.maxRam; i++) {
                ...
                if (i < this.player.ram) {
                    ...
                    this.ctx.shadowBlur = 2; this.ctx.shadowColor = ramColor; // Reduced Glow
                    this.ctx.fill();
                    this.ctx.shadowBlur = 0;
                } else {
                    // Empty (Used)
            ...
            if (this.combo > 0) {
                ...
                if (this.combo < 10) {
                    progress = this.combo / 10;
                    ringColor = '#fff';
                } else if (this.combo < 25) {
                    progress = (this.combo - 10) / 15;
                    ringColor = '#0ff';
                } else if (this.combo < 50) {
                    progress = (this.combo - 25) / 25;
                    ringColor = '#f0f';
                } else {
                    progress = 1;
                ...
                this.ctx.lineWidth = 2; // Thinner (was 4)
                this.ctx.lineCap = 'round';
                this.ctx.shadowBlur = 4; // Reduced Glow (was 10)
