...
export class EntityManager {
    ...
    spawnEnemy(x, y, type = 0) {
        if (x === undefined || y === undefined) {
            ...
            do {
                if (type === 3) {
                    // Spawn relative to CENTER (Radius: 400px max)
                    const angle = Math.random() * Math.PI * 2;
                    const dist = 50 + Math.random() * 350; // 50 to 400px
        ...
        for (let i = this.list.length - 1; i >= 0; i--) {
            ...
            if (e.type === 3) {
                if (e.warmupTimer > 0) {
                    ...
                    // STRICT BOUNDARY ENFORCEMENT (400px Radius)
                    const cx = this.game.width / 2;
                    const cy = this.game.height / 2;
                    const distFromCenter = Math.hypot(e.x - cx, e.y - cy);
                    if (distFromCenter > 400) {
                        const angle = Math.atan2(e.y - cy, e.x - cx);
                        e.x = cx + Math.cos(angle) * 400;
                        e.y = cy + Math.sin(angle) * 400;
                        // Kill outward velocity
                        const vAngle = Math.atan2(e.vy, e.vx);
                        if (Math.abs(vAngle - angle) < Math.PI / 2) {
                            e.vx *= -0.5; // Bounce back slightly
                            e.vy *= -0.5;
                        }
                    }
