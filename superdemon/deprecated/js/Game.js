...
export const Game = {
    ...
    start() {
        ...
        this.em.queueSpawn(2); this.em.spawnMine(); this.updateUI();
    },
    ...
    triggerLevelUp() {
        ...
        // Safety: Prevent immediate clicks
        this.waitingForResume = true;
        setTimeout(() => { this.waitingForResume = false; }, 500);

        document.getElementById('upgrade-screen').classList.add('active'); AudioSys.levelup();
    },

    selectUpgrade(key) {
        if (this.waitingForResume) return;
        this.upgrades[key]++;
    ...
    executeDash() {
        ...
        this.em.slashLines.push({ x1: startX, y1: startY, x2: ex, y2: ey, life: lineLife, width: 15, lethal: this.upgrades.echo > 0 });
        ...
        if (this.upgrades.explosion > 0) {
            const radius = 70 + (this.upgrades.explosion * 20);
    ...
    triggerPrismNova(x, y) {
        this.em.shockwaves.push({ x: x, y: y, radius: 0, maxRadius: 1500, life: 40, color: '#fff', width: 50 });
    ...
    killEnemy(index, source) {
        ...
        // SCORING FIX: Nanites/Mines now increment combo
        ...
        if (this.upgrades.nanites > 0) {
            const currentNanites = this.em.particles.filter(p => p.type === 'nanite').length;
        ...
        this.em.shockwaves.push({ x: dx, y: dy, radius: 0, maxRadius: stunRadius, life: 20 });
    ...
    createExplosion(x, y, radius) {
        this.em.shockwaves.push({ x: x, y: y, radius: 0, maxRadius: radius, life: 15, color: '#f80' });
        AudioSys.explode(this.combo);
    },

    update(dt) {
        // Base time unit: 1.0 = 60fps frame (approx 0.016s)
        // dt is in seconds.
        // We want to maintain current tuning which assumes 1 frame per update.
        // So we multiply by 60 to get "frames passed".
        ...
        this.grid.update(); // Grid might need dt too, but let's leave it for now or pass it if needed.
        ...
        if (this.upgrades.voltage > 0 && this.aim.active) {
            this.player.aimTimer += scaledDt; const radius = 120 + (this.upgrades.voltage * 40);
        ...
        if (this.finalBossActive) {
            ...
            if (this.finalZoneRadius > 300) {
                this.finalZoneRadius -= 0.2 * scaledDt; // Slow Shrink
            }
        ...
        // Singularity Pull Logic moved to Entities.js
        ...
        // Mine Spawning
        let mineRate = 0.005;
        if (this.finalBossActive) mineRate = 0.05; // 10x spawn rate during ritual
            ...
            if (l.lethal && l.life > 5) {
                for (let j = this.em.list.length - 1; j >= 0; j--) {
                    ...
                    if (this.lineCircleCollide(l.x1, l.y1, l.x2, l.y2, e)) {
                        ...
                        this.killEnemy(j, 'echo'); this.em.spawnText(e.x, e.y, "ECHO", `hsl(${this.hue}, 100%, 50%)`);
                        break;
                    }
                }
            }
    ...
    draw() {
        ...
        this.grid.draw(this.ctx, this.hue, this.width / 2, this.height / 2, this.combo);

        // Final Boss Zone
        if (this.finalBossActive) {
            this.ctx.save();
            this.ctx.globalCompositeOperation = 'source-over'; // Draw solid lines over additive background
            this.ctx.beginPath();
            ...
            // Draw Negative Space (Death Zone)
            this.ctx.globalCompositeOperation = 'source-over';
        ...
        this.em.particles.forEach(p => {
            ...
            if (p.type === 'confetti') {
                ...
                this.ctx.shadowBlur = 0; // Removed glow for perf
                this.ctx.fillRect(-1, -1, 2, 2); // Tiny square
