export const AudioSys = {
    ...
    startHum() {
        ...
        try {
            ...
            this.choirOsc1.frequency.value = 880.00; // A5
            this.choirOsc2.frequency.value = 1108.73; // C#6
            this.choirOsc3.frequency.value = 1661.22; // G#6 (Major 7th - Cheerful Tension)

            this.shimmerLFO.type = 'sine';
            this.shimmerLFO.frequency.value = 5.0; // Slightly slower vibrato

            const shimmerGain = this.ctx.createGain();
            shimmerGain.gain.value = 4.0; // Slightly less depth
            this.shimmerLFO.connect(shimmerGain);
            shimmerGain.connect(this.choirOsc3.frequency);

            this.choirFilter.type = 'lowpass';
            this.choirFilter.frequency.value = 2000; // Opened up for shimmer
    ...
    updateHum(combo) {
        ...
        try {
            ...
            const targetVol = 0.05 + (intensity * 0.1);
            this.humGain.gain.setTargetAtTime(targetVol, t, 0.1);
            ...
            this.humOscs.forEach((osc, i) => {
                if (this.humBaseFreqs[i]) {
                    osc.frequency.setTargetAtTime(this.humBaseFreqs[i] * pitchMult, t, 0.1);
                }
            });
            ...
            // --- ANGELIC VOCAL UPDATE (At 60x) ---
            if (combo >= 60) {
                // Fade in Vocal
                this.choirGain.gain.setTargetAtTime(0.15, t, 2.0); // Reduced volume, fixed level
                // Slight pitch drift for realism
                this.choirOsc1.frequency.setTargetAtTime(880.00 + Math.sin(t) * 2, t, 0.1);
                this.choirOsc2.frequency.setTargetAtTime(1108.73 + Math.cos(t) * 2, t, 0.1);
                // Shimmer follows
                this.choirOsc3.frequency.setTargetAtTime(1661.22, t, 0.1);
            } else {
                // Fade out Vocal
                this.choirGain.gain.setTargetAtTime(0, t, 0.5);
            }
    ...
    playHeartbeat() {
        ...
        try {
            ...
            osc.type = 'sine';
            osc.frequency.setValueAtTime(150, t); // Sharper attack
            osc.frequency.exponentialRampToValueAtTime(50, t + 0.08); // Faster drop

            gain.gain.setValueAtTime(1.2, t);
            gain.gain.exponentialRampToValueAtTime(0.01, t + 0.08); // Faster decay
    ...
    nova(combo = 0) {
        ...
        try {
            ...
            osc1.type = 'triangle'; const freq = 100 + (cappedCombo * 10); osc1.frequency.setValueAtTime(freq, t); osc1.frequency.exponentialRampToValueAtTime(10, t + 1.0); gain1.gain.setValueAtTime(0.8, t); gain1.gain.exponentialRampToValueAtTime(0.01, t + 1.0);
            ...
            gain2.gain.setValueAtTime(0.3, t); gain2.gain.linearRampToValueAtTime(0, t + 0.5);
    ...
    explode(combo = 0) {
        ...
        try {
            ...
            const cappedCombo = Math.min(combo, 100);
            const osc1 = this.ctx.createOscillator(); const gain1 = this.ctx.createGain();
            osc1.type = 'triangle'; const baseFreq = 60 + Math.min(cappedCombo * 5, 150); osc1.frequency.setValueAtTime(baseFreq * 2, t); osc1.frequency.exponentialRampToValueAtTime(baseFreq * 0.5, t + 0.4); gain1.gain.setValueAtTime(0.5, t); gain1.gain.exponentialRampToValueAtTime(0.01, t + 0.4);

            const osc2 = this.ctx.createOscillator(); const gain2 = this.ctx.createGain(); const filter = this.ctx.createBiquadFilter();
            osc2.type = 'sawtooth'; osc2.frequency.setValueAtTime(baseFreq * (1 + (cappedCombo % 5) * 0.2), t); osc2.frequency.exponentialRampToValueAtTime(10, t + 0.2);
            filter.type = 'lowpass'; filter.frequency.setValueAtTime(300 + (cappedCombo * 100), t); filter.frequency.linearRampToValueAtTime(50, t + 0.3);
            gain2.gain.setValueAtTime(0.3, t); gain2.gain.exponentialRampToValueAtTime(0.01, t + 0.2);

            osc1.connect(gain1); gain1.connect(this.ctx.destination);
            osc2.connect(filter); filter.connect(gain2); gain2.connect(this.ctx.destination);
            osc1.start(); osc1.stop(t + 0.4); osc2.start(); osc2.stop(t + 0.3);
        } catch (e) { }
    },
