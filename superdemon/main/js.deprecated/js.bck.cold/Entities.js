...
export class EntityManager {
    ...
    spawnEnemy(x, y, type = 0) {
        if (x === undefined || y === undefined) {
            ...
            if (type === 3) {
                // Scale margin with Fisheye Distortion (Power Tier)
                // Base 150 + 50 per tier (Max ~300 at Tier 3)
                margin = 150 + (this.game.powerTier * 50);
            }
        ...
        let e = { x: x, y: y, vx: 0, vy: 0, radius: 20, color: '#f0f', speed: 3.75, hasShield: false, shieldAngle: 0, stunned: 0, invuln: 0, type: type, timer: 0, opacity: 1 };
        ...
        for (let i = this.list.length - 1; i >= 0; i--) {
            ...
            if (e.type === 3) {
                if (e.warmupTimer > 0) {
                    ...
                    this.game.grid.applyForce(e.x, e.y, pullRange, -5);

                    if (dSq < pullRangeSq) {
                        const force = 0.35 * timeScale;
                        this.game.player.vx += (dx / d) * force; this.game.player.vy += (dy / d) * force;
                    }
                }
            }
