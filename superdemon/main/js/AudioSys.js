export const AudioSys = {
    ...
    dash(combo = 0) {
        ...
        try {
            ...
            // Ominous Phaser Dash (Gravitas)
            const osc = this.ctx.createOscillator();
            const gain = this.ctx.createGain();
            const filter = this.ctx.createBiquadFilter();

            // Triangle for "Sine-like" but with enough harmonics for the filter
            osc.type = 'triangle';

            // Pitch: Lower start for more weight
            const startFreq = 250 + (cappedCombo * 5);
            osc.frequency.setValueAtTime(startFreq, t);
            osc.frequency.exponentialRampToValueAtTime(60, t + 0.5); // Deeper drop

            // Filter: Darker sweep for "Gravitas"
            filter.type = 'lowpass';
            filter.Q.value = 6;
            filter.frequency.setValueAtTime(800 + (cappedCombo * 20), t); // Reduced from 3000 (Much darker)
            filter.frequency.exponentialRampToValueAtTime(50, t + 0.5);

            // Volume Envelope (Boosted)
            ...
            // Secondary "Sub" layer for modern weight (Boosted)
            const subOsc = this.ctx.createOscillator();
            const subGain = this.ctx.createGain();
            subOsc.type = 'sine';
            subOsc.frequency.setValueAtTime(60, t);
            subOsc.frequency.linearRampToValueAtTime(30, t + 0.5);
    ...
    jousterCharge() {
        ...
        try {
            ...
            osc.frequency.linearRampToValueAtTime(1200, t + 0.5); // Rising "Glint"
    ...
    jousterDash() {
        ...
        try {
            ...
            osc.type = 'sine'; // Cleaner tone
            osc.frequency.setValueAtTime(800, t);
            osc.frequency.exponentialRampToValueAtTime(50, t + 0.2); // Fast drop

            filter.type = 'lowpass';
            filter.Q.value = 10; // High Resonance for "Laser" Zap
    ...
    overheat() {
        ...
        try {
            ...
            // Devastating Power Down (Low Saw)
            osc.type = 'sawtooth';
            osc.frequency.setValueAtTime(100, t);
            osc.frequency.exponentialRampToValueAtTime(20, t + 1.0); // Slow, deep drop

            gain.gain.setValueAtTime(0.8, t);
            gain.gain.exponentialRampToValueAtTime(0.01, t + 1.0);

            // Steam Hiss (White Noise)
            ...
            noiseGain.gain.exponentialRampToValueAtTime(0.01, t + 0.5); // Short burst
    ...
    playStartSound() {
        ...
        try {
            ...
            // "Super Bassy Pew" - Low Pass Filtered Sawtooth
            const osc = this.ctx.createOscillator();
            const gain = this.ctx.createGain();
            const filter = this.ctx.createBiquadFilter();

            osc.type = 'sawtooth'; // Rich base for filtering

            // Pitch Drop (The "Pew")
            // Start low-mid (100Hz) and drop to sub-bass (30Hz)
            osc.frequency.setValueAtTime(120, t);
            osc.frequency.exponentialRampToValueAtTime(30, t + 0.8); // Fast deep drop

            // Filter Sweep (The "Warmth")
            filter.type = 'lowpass';
            filter.Q.value = 1; // Slight resonance for "body"
            // Start somewhat open (300Hz) but not buzzy, close down to 50Hz
            filter.frequency.setValueAtTime(300, t);
            filter.frequency.exponentialRampToValueAtTime(50, t + 0.8);

            // Volume Envelope (No Swell - Instant Hit)
            gain.gain.setValueAtTime(0.3, t); // Moderate punch
            gain.gain.exponentialRampToValueAtTime(0.01, t + 0.5); // Short tail
