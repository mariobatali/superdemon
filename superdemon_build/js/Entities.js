...
export class EntityManager {
    ...
    spawnEnemy(x, y, type = 0) {
        if (x === undefined || y === undefined) {
            ...
            do {
                if (type === 3) {
                    // Spawn relative to CENTER (Radius: 250px max)
                    const angle = Math.random() * Math.PI * 2;
                    const dist = 50 + Math.random() * 100; // 50 to 150px (Closer to center)
        ...
        if (type === 3) { e.color = '#FF4500'; e.radius = 30; e.speed = 0; e.warmupTimer = 60; } // Gravity: Orange-Red
            ...
            e.hasShield = true; e.color = '#0ff'; e.shieldAngle = Math.random() * Math.PI * 2; e.speed = 2.5;
        }
        else if (type === 6) { // Jouster
            e.color = '#f00'; e.radius = 20; e.speed = 3.0;
            e.state = 0; // 0:Tracking, 1:Charging, 2:Dashing, 3:Cooldown
            e.timer = 60 + Math.random() * 60;
            e.dashTarget = { x: 0, y: 0 };
    ...
    spawnBoss(x, y) {
        ...
        // this.game.wardensKilled++; // Removed: Only increment on death
        ...
        this.list.push({
            x: bx, y: by, vx: 0, vy: 0, radius: 50, color: '#fff',
    ...
    fireProjectile(enemy) {
        ...
        this.projectiles.push({ x: enemy.x, y: enemy.y, vx: (dx / dist) * 3, vy: (dy / dist) * 3, radius: 7.5, life: 300 }); // Radius 5 -> 7.5
        AudioSys.shoot();
    }
    ...
    update(dt, realDt) {
        const timeScale = Math.min(dt, 2.0); // Clamp dt to prevent explosion
        ...
        for (let i = this.list.length - 1; i >= 0; i--) {
            let e = this.list[i];
            const safeX = e.x; const safeY = e.y; // Capture safe state

            // SANITIZATION: Prevent NaN/Infinity from breaking the game
            if (!Number.isFinite(e.x) || !Number.isFinite(e.y) || !Number.isFinite(e.vx) || !Number.isFinite(e.vy)) {
                // Reset to random position near center to avoid stacking on player
                const angle = Math.random() * Math.PI * 2;
                const dist = 100 + Math.random() * 100;
                e.x = (this.game.width / 2) + Math.cos(angle) * dist;
                e.y = (this.game.height / 2) + Math.sin(angle) * dist;
                e.vx = 0; e.vy = 0;
                // Reset Jouster specific state to prevent loop
                if (e.type === 6) {
                    e.state = 3; // Cooldown
                    e.timer = 60;
                    e.dashVx = 0; e.dashVy = 0;
            ...
            if (d > 0) {
                ...
                // GLOBAL TETHER (Screen Bounds)
                // If off-screen, rush back to center at 5x speed
                // EXCEPTION: Jousters Dashing (State 2) are allowed to leave temporarily
                if ((e.type !== 6 || e.state !== 2) && (e.x < 0 || e.x > this.game.width || e.y < 0 || e.y > this.game.height)) {
                    ...
                    if (td > 0) {
                        e.vx = (tdx / td) * e.speed * 5;
                        e.vy = (tdy / td) * e.speed * 5;
                    }
                }
            }
            ...
            if (e.type === 1) { // Shooter
                e.timer -= 1 * timeScale;
                if (e.timer <= 0) { this.fireProjectile(e); e.timer = 300; } // Reduced fire rate (was 150)

                let fireRate = 200; // Reduced fire rate (was 100)
                if (this.game.bossEncounters > 1) fireRate = 120; // Reduced (was 60)
                if (this.game.bossEncounters > 3) fireRate = 60; // Reduced (was 30)
            ...
            if (e.type === 3) {
                if (e.warmupTimer > 0) {
                    ...
                    if (distFromCenter > 300) {
                        // Rush back to center
                        const angleToCenter = Math.atan2(cy - e.y, cx - e.x);
                        e.vx = Math.cos(angleToCenter) * e.speed * 5;
                        e.vy = Math.sin(angleToCenter) * e.speed * 5;
                    }

                    // HARD CLAMP (350px Safety Net)
                    if (distFromCenter > 350) {
                        const angle = Math.atan2(e.y - cy, e.x - cx);
                        e.x = cx + Math.cos(angle) * 350;
                        e.y = cy + Math.sin(angle) * 350;
                    }
            ...
            // Jouster Logic (Type 6)
            if (e.type === 6) {
                if (e.state === 0) { // Tracking
                    e.timer -= timeScale;
                    if (d > 1) { // Avoid division by zero
                        e.vx += (dx / d) * 0.15 * timeScale;
                        e.vy += (dy / d) * 0.15 * timeScale;
                        const s = Math.hypot(e.vx, e.vy);
                        if (s > e.speed && s > 0.001) { // Avoid NaN
                            e.vx = (e.vx / s) * e.speed;
                            e.vy = (e.vy / s) * e.speed;
                        }
                        e.x += e.vx * timeScale; e.y += e.vy * timeScale;
                    ...
                    if (e.timer <= 0) {
                        e.state = 1; // Charge
                        e.timer = 75; // Longer Charge (was 45)
                        e.vx *= 0.1; e.vy *= 0.1; // Stop
                        AudioSys.warn();
                    }
                } else if (e.state === 1) { // Charging
                    e.timer -= timeScale;
                    e.x += e.vx * timeScale; e.y += e.vy * timeScale; // Drift
                    // Lock target
                    e.dashTarget.x = this.game.player.x;
                    e.dashTarget.y = this.game.player.y;
                    if (e.timer <= 0) {
                        e.state = 2; // Dash
                        e.timer = 15; // Increased duration (was 10) for visibility
                        const ddx = e.dashTarget.x - e.x;
                        const ddy = e.dashTarget.y - e.y;
                        const dd = Math.hypot(ddx, ddy);
                        const dashSpeed = 40; // Reduced speed (was 60) to prevent tunneling/skipping

                        // Visual Dash Line
                        this.slashLines.push({
                            x1: e.x, y1: e.y,
                            x2: e.dashTarget.x, y2: e.dashTarget.y,
                            life: 20, width: 10, color: '#f00', lethal: false
                        });

                        if (dd > 0.1) {
                            e.dashVx = (ddx / dd) * dashSpeed;
                            e.dashVy = (ddy / dd) * dashSpeed;
                        } else {
                            e.dashVx = dashSpeed; e.dashVy = 0;
                        }
                        // Safety: Ensure dash velocity is valid
                        if (!Number.isFinite(e.dashVx) || !Number.isFinite(e.dashVy)) {
                            e.dashVx = dashSpeed; e.dashVy = 0;
                        }
                        AudioSys.jousterDash(); // Sharp sound
                    }
                } else if (e.state === 2) { // Dashing
                    e.timer -= timeScale;
                    const oldX = e.x; const oldY = e.y;
                    e.vx = e.dashVx; e.vy = e.dashVy;
                    e.x += e.vx * timeScale; e.y += e.vy * timeScale;

                    // TUNNELING FIX: Check collision along path
                    const pdx = this.game.player.x - oldX;
                    const pdy = this.game.player.y - oldY;
                    const pDist = Math.hypot(pdx, pdy);
                    const stepDist = Math.hypot(e.x - oldX, e.y - oldY);

                    // Simple line check: if player is close to the segment
                    if (stepDist > 0.1 && pDist < stepDist + 50) { // Optimization check
                        // Project player onto line segment
                        const t = ((this.game.player.x - oldX) * (e.x - oldX) + (this.game.player.y - oldY) * (e.y - oldY)) / (stepDist * stepDist);
                        const clampedT = Math.max(0, Math.min(1, t));
                        const closestX = oldX + clampedT * (e.x - oldX);
                        const closestY = oldY + clampedT * (e.y - oldY);
                        const distToLine = Math.hypot(this.game.player.x - closestX, this.game.player.y - closestY);

                        if (distToLine < e.radius + this.game.player.radius) {
                            this.game.triggerGameOver("FATAL EXCEPTION");
                        }
                    }

                    // Trail Effect
                    if (Math.random() < 0.5) {
                        this.spawnGhost(e.x, e.y, '#f00', e.radius);
                    }

                    if (e.timer <= 0) {
                        e.state = 3; // Cooldown
                        e.timer = 60;
                        e.vx = 0; e.vy = 0; // Fix: Stop momentum immediately
                    }
                } else if (e.state === 3) { // Cooldown
                    e.timer -= timeScale;
                    // e.vx *= 0.9; e.vy *= 0.9; // Friction no longer needed if we stop them
                    // e.x += e.vx * timeScale; e.y += e.vy * timeScale;
                    if (e.timer <= 0) {
                        e.state = 0; // Back to Tracking
                        e.timer = 60 + Math.random() * 60;
                    }
                }
            }
            ...
            if (!Number.isFinite(e.x) || !Number.isFinite(e.y)) {
                e.x = safeX; e.y = safeY; // Revert to last known good position
                e.vx = 0; e.vy = 0; // Stop movement
                if (e.type === 6) { e.state = 3; e.timer = 60; } // Force Cooldown
            }
            ...
            if (dSq < collisionDist * collisionDist && e.stunned <= 0) {
                ...
                if (e.type === 6) reason = "FATAL EXCEPTION";
